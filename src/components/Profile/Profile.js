    ...
    if (this.state.profileName) {
      ...
      .then(response => {
        if (response.data.status) {
          ...
          axios.get(userUrl + profileName)
          .then(response => {
            let profile = response.data[0];
            this.setState({ profile: profile, _fics: profile._fics, _chapters: profile._chapters, authenticated: true });
          })
          .catch(error => {
            console.log(error)
          })
  ...
  render() {
    ...
    if (this.state.isLoading) {
      ...
      if (!this.state.profileName && !this.state.authenticated) {
        return (
          <Header className="not-auth-header" as="h1" inverted>You need to be authenticated to view this page =(</Header>
        )
      } else {
        return (
          <div className="fic">
            <Grid stackable columns={2} divided inverted>
              ...
              <Grid.Column width={4} only="mobile tablet computer">
                ...
                {(this.state.authenticated) ? <Button style={{ marginTop: 1 + "rem" }} fluid color="violet" inverted active>Edit Profile</Button> : <div></div>}
              </Grid.Column>
